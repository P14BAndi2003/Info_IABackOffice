...
@endif
    <form action="{{url('/confirmerModifier')}}" method="POST" enctype="multipart/form-data">
        @csrf

        <div class="form-group">
            <label for="titre">Titre :</label>
            <input type="text" name="titre" id="titre" value="{{ $article->titre }}" class="form-control" required>
        </div>
        <div class="form-group">
            <label for="resume">Résumé :</label>
            <input type="text" name="description" id="description" value="{{ $article->description }}" class="form-control" required>
        </div>
        <input type="hidden" name="id" value={{ $article->id }}>
        <div class="form-group">
            <label for="image">Image : (veuillez re-sélectionner l'image)</label>
            <input name="image"  type="file" id="image" class="form-control-file form-control height-auto">
            <img id="photo-preview" src="data:image/png;base64,{{ $article->image }}" alt="Aperçu de la photo">
        </div>
         
        <div class="form-group">
            <label for="contenu">Contenu :</label>
            <textarea name="contenu" class="ckeditor" rows="10" required>{{ $article->contenu }}</textarea>
        </div>
